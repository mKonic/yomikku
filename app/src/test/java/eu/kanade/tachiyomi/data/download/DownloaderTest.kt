...
/**
 * Pausing for a lost network cancels downloads that are still cleaning up. A restart must wait for that cleanup, and
 * must not re-queue or re-download a chapter the cleanup finished (mihonapp/mihon#3500).
 */
class DownloaderTest {
    ...
    private fun downloader(): Downloader {
        every { sourceManager.isInitialized } returns MutableStateFlow(true)
        val parallelSources = mockk<Preference<Int>>()
        every { parallelSources.changes() } returns MutableStateFlow(1)
        every { preferences.parallelSourceLimit() } returns parallelSources
        return Downloader(
            context = mockk(),
            provider = mockk(),
            cache = mockk(),
